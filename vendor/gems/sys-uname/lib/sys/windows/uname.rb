...
module Sys
  ...
  class Uname
    ...
    def self.fallback_architecture
      arch = ENV['PROCESSOR_ARCHITEW6432'].to_s
      arch = ENV['PROCESSOR_ARCHITECTURE'].to_s if arch.empty?
      arch = RbConfig::CONFIG['host_cpu'].to_s if arch.empty?

      normalized_arch = arch.downcase

      case normalized_arch
      when /amd64|x86_64/
        'x86_64'
      when /arm64|aarch64/
        'arm64'
    ...
    private_class_method :fallback_architecture

    def self.fallback_machine
      host_cpu = RbConfig::CONFIG['host_cpu'].to_s
      return host_cpu unless host_cpu.empty?

      fallback_architecture
    end

    private_class_method :fallback_machine
  end
end
