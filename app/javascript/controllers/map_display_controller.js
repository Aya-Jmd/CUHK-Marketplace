...
export default class extends Controller {
  ...
  initMap() {
    ...
    if (!lat || !lng) {
      this.element.innerHTML = '<p class="text-muted">No location set for this item</p>'
      return
    }

    const map = L.map(this.element).setView([lat, lng], 16)
    
    // Restrict view to CUHK area
    ...
    const locationDisplay = this.locationNameValue ? 
      this.locationNameValue.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : 
      'Pickup location'
